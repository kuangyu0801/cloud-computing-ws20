...
public interface INotebookDAO {
     ...
     * @return The note with the given ID (with text), or null if the ID does
     *         not exist.
     */
    public NoteWithText getNote(String noteID);

    /**
     * Creates or updates a note. If the note contains no ID, a new note is
     * created and an ID is assigned to it. If the note contains an ID, the
     * corresponding note is updated (overwritten). If there exists no note with
     * the given ID, a new note is created and the given ID may be replaced by a
     * different ID.
     * 
     * @param note
     ...
     * 
     * @param noteID
     *            The ID of the note to delete. If the ID does not exist,
     *            nothing happens.
     */
